...
function Card({ children }) {
  return (
    <div style={{ background: 'var(--paper)', border: '1px solid var(--line-2)', borderRadius: 10, padding: '28px 28px 24px' }}>
      {children}
    </div>
  );
}
...
function BrandHeader() {
  return (
    <div className="mb-7 flex flex-col items-center text-center">
      ...
      <span style={{ display: 'inline-block', fontSize: 10, fontWeight: 600, letterSpacing: '.07em', textTransform: 'uppercase', color: 'var(--mute)', background: 'var(--line-2)', borderRadius: 4, padding: '2px 8px' }}>
        Operator Panel
      </span>
    </div>
  );
...
export default function VerifyEmail() {
  ...
  const ResendForm = () => resendSent ? (
    <p className="mt-4 rounded-md px-3 py-2 text-center" style={{ fontSize: 13, color: 'var(--ok)', background: 'rgba(34,197,94,.07)' }}>
    ...
    <form onSubmit={handleResend} className="mt-5 space-y-3">
      <input
        ...
        required
      />
      {resendError && <p style={{ margin: 0, fontSize: 12, color: 'var(--bad)' }}>{resendError}</p>}
      <button type="submit" disabled={resendLoading} className="btn-primary" style={{ width: '100%', height: 38, justifyContent: 'center' }}>
        {resendLoading ? <><Loader2 size={13} className="animate-spin" /> Sending…</> : 'Resend verification email'}
      </button>
    </form>
  );

  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-4 py-8" style={{ background: 'var(--paper-2)' }}>
          ...
          <Card>
            <div className="flex flex-col items-center gap-3 py-2 text-center">
              ...
              <p style={{ margin: 0, fontSize: 13, color: 'var(--mute)' }}>
                Your operator account is now fully active. Sign in to continue.
              </p>
